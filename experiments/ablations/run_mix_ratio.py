#!/usr/bin/env python3
"""Mix ratio ablation — test different label:CoT ratios for C_mix.
Usage:
  torchrun --nproc_per_node=4 experiments/ablations/run_mix_ratio.py --ratio 0.25
  python experiments/ablations/run_mix_ratio.py --ratio 0.25 --eval-only
"""
...
from src.student_training import train_mixed
...
def run(ratio: float, eval_only=False):
    exp_name = f"mix_{int(ratio*100)}_{int((1-ratio)*100)}"
    ...
    cfg_copy["student"]["training"]["mix_ratio"] = ratio
    ...
    if not eval_only:
        logger.info(f"Mix ratio ablation: label={ratio:.0%} / CoT={1-ratio:.0%}")
        result = train_mixed(cfg_copy, seed=cfg["project"]["seed"])
        if result is None:
            return None
        final_dir = result
    ...
    metrics["mix_ratio"] = ratio
    logger.info(f"Mix {ratio:.0%}/{1-ratio:.0%}: Macro F1={metrics['macro_f1']:.4f}")
